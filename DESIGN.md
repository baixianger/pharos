...
- **Delivery model (mention-only, 2026-08-13).** `@mention` = DIRECTED: the
  named agents get the message and are poked. No mention = TRANSCRIPT ONLY: the
  message is visible in room history but enters no agent mailbox and never
  triggers a Stop hook. This prevents ordinary human room chatter from blocking
  every agent. The stored `MeshMsg.to` remains the discriminator: non-empty means
  directed delivery; empty means transcript-only.
