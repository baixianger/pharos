...
<dict>
    ...
    <!-- SUPublicEDKey: paste your EdDSA public key here (generated by Sparkle's ./bin/generate_keys) -->
...
# SwiftPM resolves Sparkle as a binary XCFramework and copies the correct
# architecture slice into the build products directory as Sparkle.framework.
...
# Sparkle ships nested code: Updater.app and Downloader.xpc live inside
# Sparkle.framework — sign them (deepest first) before the parent framework.
sign_frameworks() {
  local fw
  for fw in "$APP/Contents/Frameworks/"*.framework; do
    ...
    # Sign nested XPC services (e.g. Sparkle's Downloader.xpc).
    local xpc_dir
    for xpc_dir in "$fw/Versions/"*/XPCServices/*.xpc; do
      [[ -d "$xpc_dir" ]] && codesign "${CODESIGN_ARGS[@]}" "$xpc_dir"
    done
    # Sign nested app bundles (e.g. Sparkle's Updater.app).
    local nested_app
    for nested_app in "$fw/Versions/"*/*.app; do
      [[ -d "$nested_app" ]] && codesign "${CODESIGN_ARGS[@]}" "$nested_app"
    done
    # Sign remaining standalone executables inside the framework.
