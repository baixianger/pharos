...
public enum DistributedHookCLI {
    ...
    public static func hostPresenceSnapshot(
        ...
        for (memberID, presence) in localAgentPresence(rootURL: replica.rootURL) {
            ...
            if resource == nil,
               !memberships.isEmpty {
                // Repairs memberships created by pre-2.0.0 join paths while
                // preserving the same Host ownership boundary used for new
                // sessions.
                try await chat.ensureLocalPresenceAuthority(memberID: memberID)
                resource = try await replica.store.hostResource(
                    in: group, hostDeviceID: replica.identity.deviceID,
                    resourceID: resourceID
                )
            }
            if let legacy = resource, legacy.state == .active,
               legacy.hostEndpointID == hostEndpointID,
               !legacy.allowedActions.contains(.presence) {
                resource = try await replica.store.replaceHostResource(
                    in: group, on: replica.identity, resourceID: resourceID,
                    allowedActions: Set(legacy.allowedActions).union([.presence]),
                    at: max(
                        legacy.updatedAt,
                        MeshHybridTimestamp(
                            wallTimeMilliseconds: nowMilliseconds
