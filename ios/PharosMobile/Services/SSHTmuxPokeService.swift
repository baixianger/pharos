...
enum TmuxPokeCommand {
    static func build(nick: String, memberID: String, pane paneID: String, kind: String?) throws -> String {
...
actor SSHTmuxPokeService {
    func poke(member: MeshMember, profile: SSHHostProfile, privateKey: Curve25519.Signing.PrivateKey) async throws {
        ...
        let command = try TmuxPokeCommand.build(nick: member.nick, memberID: member.id, pane: pane, kind: member.kind)
